...
//                restartZrtpControl();
     ...
     * argument.
     *
     * @param baf the {@link ByteArrayBuffer} that holds the RTP packet.
     *
     ...
     */
    public boolean supportsFrameBoundaries(ByteArrayBuffer baf)
    {
        if (frameMarkingsExtensionId == -1)
        {
            REDBlock redBlock = getPrimaryREDBlock(baf);
            if (redBlock != null && redBlock.getLength() != 0)
            {
                final byte vp9PT = getDynamicRTPPayloadType(Constants.VP9),
